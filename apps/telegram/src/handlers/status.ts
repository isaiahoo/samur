...
export async function sendStatus(bot: TelegramBot, chatId: number, fromId: number, fromName: string): Promise<void> {
  try {
...
export function registerStatusHandler(bot: TelegramBot): void {
  bot.onText(/\/status/, async (msg) => {
    const name =
      ...
      "Пользователь";
    await sendStatus(bot, msg.chat.id, msg.from!.id, name);
  });
}
