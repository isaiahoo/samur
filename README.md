# Кунак — Платформа координации взаимопомощи при наводнении

Платформа реального времени для координации спасательных операций и взаимопомощи при наводнении на Северном Кавказе (апрель 2026).

> **Внутренний код-name проекта остался `samur`** — пакеты (`@samur/*`), контейнеры (`samur-*`), база (`POSTGRES_DB=samur`), домен старого сертификата (`/etc/letsencrypt/live/samur/`) — чтобы не требовать миграции данных при переименовании бренда. Любые пользовательские упоминания бренда — **Кунак / Kunak**; домен **mykunak.ru** (samurchs.ru поддерживается как алиас).
